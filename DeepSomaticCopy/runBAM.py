...
def fullRenameProcess(inFile, outFile, useCB, otherSam=False, cellConfigFile=None):
    # otherSam = True
    # print (inFile)
    ...
    if useCB == 'CB':
        ...
        # It's already split!
        fnames = os.listdir(folder0)
        cellList = fnames
        if cellConfigFile is not None:
            # open .tsv file
            content = pd.read_csv(cellConfigFile, sep='\t')
            goodCells = np.array(content[content.use].sampleID)
            cellList = goodCells
        print(cellList)
        for a in range(len(cellList)):
            print(a, len(cellList))
            filename_bam, filename_bai = [s for s in fnames if cellList[a] + '.' in s]
            command1 = 'cp ' + folder0 + '/' + filename_bam + ' ' + folder2 + filename_bam
            os.system(command1)
            command1 = 'cp ' + folder0 + '/' + filename_bai + ' ' + folder2 + filename_bai
            os.system(command1)
...
def runAllSteps(bamLoc, refLoc, outLoc, refGenome, useCB='', cellConfigFile=None):
    otherSam = False
    if useCB in ['CB', 'seperateBAMs']:
        bamLocNew = bamLoc.split('.')
        bamLocNew_end = bamLocNew[-1]
        bamLocNew = '/'.join(bamLoc.split('/')[:-1]) + '.' + bamLocNew_end

        fullRenameProcess(bamLoc, bamLocNew, useCB, otherSam=otherSam, cellConfigFile=cellConfigFile)
    ...
    stepName += 1
    stepString = str(stepName) + '/' + numSteps
    print('Data processing — Step ' + stepString + ': Running bcftools on pseudobulk (may take hours to days)... ',
          end='')
